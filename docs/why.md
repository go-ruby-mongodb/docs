...
`go-ruby-mongodb/mongodb` reimplements Ruby's `mongodb` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-mongodb/mongodb` directly, with no Ruby runtime;
